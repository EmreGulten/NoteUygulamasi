    ...
    public class NoteController : Controller
    {
        ...
        public ActionResult MyLikedNotes()
        {
            var notes = likedManager.ListQueryable().Include("LikedUser").Include("Note")
                .Where(x => x.LikedUser.Id == CurrentSession.User.Id).Select(x => x.Note).Include("Category")
        ...
        public ActionResult Edit(int? id)
        {
            ...
            ViewBag.CategoryId = new SelectList(CacheHelper.GetCategoriesFromCache(), "Id", "Title", note.CategoryId); // TODO :cashden çekilecek
            return View(note);
        }
        ...
        public ActionResult GetLiked(int[] ids)
        {
            if (CurrentSession.User != null)
            {
                List<int> likedNoteIds = likedManager
                    .List(x => x.LikedUser.Id == CurrentSession.User.Id && ids.Contains(x.Note.Id)).Select(x => x.Note.Id)
                    .ToList();
        ...
        public ActionResult SetLikeState(int noteid, bool liked)
        {
            ...
            if (res > 0)
            {
                ...
                return Json(new
                    { hasError = false, erorMessage = string.Empty, result = note.LikeCount });
            }

            return Json(new
                {hasError = true, erorMessage = "Beğenme işlemi gerçekleştirilemedi.", result = note.LikeCount});
        }
